...
export default function post({ parms}:{parms:{id:string}}){
  const {id} = parms;
  ...
  return(
    <div className='max-w-3xl mx-auto p-5'>
      <h1 className='md:text-4xl text-3xl font-bold text-red-600 text-center'>
        {" "}{post.title}</h1>
        {post.image && (
          <Image src={post.image} alt={post.title} className='w-full h-auto rounded-md mt-4'/>
        )}
        <div className='mt-6 text-lg text-slate-700'>
          { Paragraph(post.description)}
        </div>
      <CommentSec postId={post.id}></CommentSec>
      <Auther></Auther>
    </div>
  )
}
